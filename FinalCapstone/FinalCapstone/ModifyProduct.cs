...
{
    public partial class ModifyProduct : MaterialForm
    {
        ...
        public ModifyProduct(int productID)
        {
            ...
            AllPartsListView.MultiSelect = false;
            PartsWithProductListView.MultiSelect = false;
            ...
            DataTable d2 = dBQueries.PartswithProducts(ID);
            d2.DefaultView.Sort = "PartID ASC";
            ...
            foreach (DataRow row in d2.Rows)
            {
                ...
                PartsWithProductListView.Items.Add(listViewItem);
            }
        ...
        private void getProductName(int ID)
        {
            string productName = dBQueries.ProductName(ID);
            ProductName.Text = productName;
        }

        private void getProductPrice(int ID)
        {
            decimal productPrice = dBQueries.ProductPrice(ID);
            ProductPrice.Text = productPrice.ToString();
        }

        private void getProductInventory(int ID)
        {
            int productInventory = dBQueries.ProductInventory(ID);
            ProductInventory.Text = productInventory.ToString();
        }
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            if (ProductName.Text != string.Empty && ProductPrice.Text != string.Empty && ProductInventory.Text != string.Empty)
            {
                ...
                //{
                    ...
                    int ID = Product.CurrentProductID;
                    dBQueries.ModifyProduct(product, ID);
                    dBQueries.DeleteProductParts(ID);
                    bool isAdded = dBQueries.UpdateProductParts(ID, partIDs);
                    if (isAdded)
                    {
